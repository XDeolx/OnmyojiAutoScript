...
class ChessClickProgressTest(unittest.TestCase):
    ...
    @patch('tasks.Chess.script_task.time.sleep')
    def test_confirmed_rightmost_sale_clears_click_history(self, _sleep):
        task = self._task()
        task._hand_card_detections = Mock(return_value=[{'roi': (10, 20, 30, 40)}])
        task.classify_hand_card = Mock(
            return_value={'type': 'shikigami', 'name': 'kaoru'},
        )
        task._hand_card_identity_count = Mock(side_effect=[2, 1])
        task.sell_hand_card = Mock()

        result = task.sell_rightmost_hand_card()

        self.assertIsNotNone(result)
        task.device.click_record_clear.assert_called_once_with()

    @patch('tasks.Chess.script_task.time.sleep')
    def test_unconfirmed_rightmost_sale_keeps_click_history(self, _sleep):
        task = self._task()
        task._hand_card_detections = Mock(return_value=[{'roi': (10, 20, 30, 40)}])
        task.classify_hand_card = Mock(
            return_value={'type': 'shikigami', 'name': 'kaoru'},
    ...
    def test_confirmed_shop_purchase_clears_click_history(self, _sleep):
        task = self._task()
        task._match_shop_shikigami_avatar = Mock(
            side_effect=[{'score': 0.9}, None],
        )
